...
async function sendEmails(
  ...
  if (!apiKey || !fromEmail || !toEmail) {
    if (process.env.NODE_ENV !== "production") {
      console.log(
        "[contact-form] Email config missing,  message would have been:",
      );
  ...
  try {
    ...
    if (practiceResult.error) {
      console.error(
        "[contact-form] Practice email error:",
        practiceResult.error,
      );
      return {
        ok: false,
        error: "Email naar de praktijk kon niet verstuurd worden.",
      };
    }
    if (clientResult.error) {
      // Practice already received it, so don't fail the user,  just log
      console.error(
        "[contact-form] Client confirmation error:",
        clientResult.error,
      );
    }
    ...
    return {
      ok: false,
      error: "Onverwachte fout bij het versturen van e-mails.",
    };
  }
}
